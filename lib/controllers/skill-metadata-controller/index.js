...
module.exports = class SkillMetadataController {
    ...
    deploySkillPackage(vendorId, ignoreHash, callback) {
        ...
        const skillPackageTemplatesSrc = ResourcesConfig.getInstance().getSkillMetaSrc(this.profile);

        if (!stringUtils.isNonBlankString(skillPackageTemplatesSrc)) {
            return callback('Skill package src is not found in ask-resources.json.');
        }
        if (!fs.existsSync(skillPackageTemplatesSrc)) {
            return callback(`File ${skillPackageTemplatesSrc} does not exist.`);
        }

        const viewVariables = ResourcesConfig.getInstance().getSkillMetaVariables(this.profile);
        const skillPackageSrc = path.join(CONSTANTS.FILE_PATH.HIDDEN_ASK_FOLDER, CONSTANTS.FILE_PATH.SKILL_PACKAGE.PACKAGE);
        fs.emptyDirSync(skillPackageSrc);
        fs.copySync(skillPackageTemplatesSrc, skillPackageSrc);
        const files = klawSync(skillPackageSrc).filter(item => item.path.endsWith('.json'));
        // for each json file, plug values in the template
        files.forEach(file => {
            const template = fs.readFileSync(file.path, 'utf8');
            const rendered = Mustache.render(template, viewVariables);
            fs.writeFileSync(file.path, rendered);
        });
